...
impl Processor {
    pub fn process(
        ...
        match instruction {
			...
			NftInstruction::ExtendExpiry(args) => {
...
/// - price: [u64] 
	pub fn process_add_plan(
		...
		let daky_info = next_account_info(account_info_iter)?;
		let system_program_info = next_account_info(account_info_iter)?;

		// Derive PDAs
		let (daky_pubkey, daky_bump) = Pubkey::find_program_address(
			&[b"daky", mint_info.key.as_ref(), plan_id.as_bytes().as_ref()],
			program_id,
		);
		...
		invoke_signed(
			...
			&[fee_payer_info.clone(), daky_info.clone()],
			&[&[b"daky", mint_info.key.as_ref(), plan_id.as_bytes().as_ref(), &[daky_bump]]],
		)?;
		...
		if daky_info.data_len() != Daky::LEN {
			return Err(NftError::InvalidAccountLen.into());
		}
		...
		let daky = &mut AccountPDA::new(
			&daky_info,
			try_from_slice_unchecked::<Daky>(&daky_info.data.borrow()).unwrap(),
			daky_bump,
		);
		...
		daky.data.serialize(&mut &mut daky_info.data.borrow_mut()[..])?;
...
/// - assoc_account: [Pubkey] 
	pub fn process_mint(
		...
		let (subscription_pubkey, subscription_bump) = Pubkey::find_program_address(
			&[b"daky", mint_info.key.as_ref(), plan_id.as_bytes().as_ref(), assoc_account.as_ref()],
			program_id,
		);
		...
		// Accounts Serialization
		daky.data.serialize(&mut &mut daky_info.data.borrow_mut()[..])?;		subscription.data.serialize(&mut &mut subscription_info.data.borrow_mut()[..])?;
